...
fit1 <- update(fit1_1, . ~ . - P_PBCD_LBDBSESI)
summary(fit1)
...
Approach 1 and approach 2 agrees, while approach 3 included only 4 of the 5 heavy metal tests. We will now compare the three models using ANOVA and AIC.

We note fit3 is strictly nested within fit1(=fit2). We will do an F test on the two models, and compare AIC scores.

```{r}
# fit3 has fewest variables, let's see if adding the dropped variable improves the model
anova(fit3, fit1)
AIC(fit3, fit1)
```

Adding P_CBC_LBXHCT and P_CBC_LBXHGB to fit3 does indeed significantly explain more variance in the data, fit1 also is ~30 AIC scores lower than fit3. Both evidence points to fit1 being the more statistically robust model, thus we will proceed with fit1. As a reminder, during this variable selection, we dropped P_PBCD_LBDBSESI and P_PBCD_LBDBMNSI as they did not prove to be statistically significant enough to explain the variance in the data. 
