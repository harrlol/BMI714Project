...
analysis_var_ref <- var_lib%>%filter(BMI_714_Variable_Name %in% c(hep_vars, analysis_var_vec))
...
while (end_metric < 1) {
  ...
  var_sig <- data.frame()
  for (var in curr_ind_var_set) {
    # fit a model with one variable
    formula_in <- as.formula(paste(deparse(formula(fit_iter)), "+", var))
    ...
    var_sig <- rbind(var_sig, data.frame(var = var, p_val = summary(fit)$coefficients[2,4], AIC = curr_AIC))
  
    # best var by p value
    best_var_p <- var_sig[var_sig$p_val == min(var_sig$p_val),]$var
    
    # best var by p value
    best_var_AIC <- var_sig[var_sig$AIC == min(var_sig$AIC),]$var
  }
  
  fit_iter <- update(fit_iter, as.formula(paste(". ~ . +", best_var_AIC)))
  
  # wait on Aparna on deciding this, some function on fit_iter
  end_metric <- 
}
```
